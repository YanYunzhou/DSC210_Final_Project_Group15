...
def wrapToPi(theta): # Ensure that theta is in the interval [-pi,pi]
    while theta < -np.pi:
        theta = theta + 2 * np.pi
    while theta > np.pi:
        theta = theta - 2 * np.pi
...
def inv_motion_model(u_t): #Translated command u_t to translation and rotation
...
def ekf_predict(mu, sigma, u_t, R): # Predict next state from current state and commands u_t
    theta = mu[2][0]
    ...
    mu_bar = mu + np.array([[trans * cos(theta + rot1)],
                            ...
                            [rot1 + rot2]])              #Predict the robot' state
...
def ekf_correct(mu_bar, sigma_bar, z, Q, M):
    for i in range(z.shape[1]):
        ...
        K = sigma_bar @ H_t.T @ np.linalg.inv(S) #compute the Kalman Gain and the most time-consuming step: inversion!

        mu_bar = mu_bar + (K @ (z[:2, i].reshape(2, 1) - z_hat)) #The correct step
        mu_bar[2, 0] = wrapToPi(mu_bar[2, 0])
        sigma_bar = (np.identity(3) - (K @ H_t)) @ sigma_bar #Reduce the uncertainty
